...
for subdir, dirs, files in os.walk(src_dir):
    for file in files:
        ...
        points2 = getLandmarks(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        ...
        for k in range(0, len(points2)):
            ...
            points2[k] = fbc.constrainPoint(points2[k], img.shape[1], img.shape[0])
        ...
        for idx, filter in enumerate(filters):
            ...
            if filter['morph']:
                ...
                points1_values = [list(points1.values())[0], list(points1.values())[1]]
                if len(points1_values) != len(dst_points):
                    print(f"Error: points1_values and dst_points do not have the same length. Length of points1_values: {len(points1_values)}, length of dst_points: {len(dst_points)}")
                    print(f"points1_values: {points1_values}")
                    print(f"dst_points: {dst_points}")
                    exit(1)

                if len(points1_values) == 0 or len(dst_points) == 0:
                    print(f"Error: One of the point sets is empty. Length of points1_values: {len(points1_values)}, length of dst_points: {len(dst_points)}")
                    exit(1)

                tform = fbc.similarityTransform(points1_values, dst_points)
        ...
        dest_img_path = os.path.join(dest_subdir, file)
